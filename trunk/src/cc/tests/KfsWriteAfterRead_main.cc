...
void startTesting()
{
	...
	{
		cout << "File not exist, creating: " << file << endl;
		fd = gClient->Create(file);
	}
	else
	{
		cout << "File already exist, opening: " << file << endl;
		fd = gClient->Open(file, O_RDWR | O_EXCL | O_APPEND);
	}
