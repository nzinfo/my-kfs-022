/*!
 * $Id: util.cc 153 2008-09-17 19:08:16Z sriramsrao $
 *
 * \file util.cc
 ...
 * start of the chunk.
 */
chunkOff_t KFS::chunkStartOffset(chunkOff_t offset)
{
 ...
 */
int KFS::link_latest(const string realname, const string alias)
{
	int status = 0;
 ...
 */
/// 将一个数字转换成string输出
const string KFS::toString(long long n)
{
 ...
 * \return	the number
 */
/// 将一个字符串转换成long long类型的数据输出
long long KFS::toNumber(string s)
{
	char *endptr;
 ...
 */
/// 生成文件名: dir + '/' + prefix + '.' + number
string KFS::makename(const string dir, const string prefix, int number)
{
 ...
 */
/// 将一个字符串以sep为分隔符, 分割成一个一个的小字符串, 按顺序存放在component中
void KFS::split(deque<string> &component, const string path, char sep)
{
 ...
 */
bool KFS::file_exists(string name)
{
	struct stat s;
 ...
 */
void KFS::sendtime(ostringstream &os, const string &prefix, struct timeval &t,
		const string &suffix)
{
 ...
 */
string KFS::timeToStr(time_t val)
{
	char timebuf[64];
 ...
 */
void KFS::warn(const string msg, bool use_perror)
{
	if (use_perror)
		perror(msg.c_str());
	else
 ...
 * \param[in] use_perror pass text to perror() if true
 */
void KFS::panic(const string msg, bool use_perror)
{
	warn(msg, use_perror);
	abort();
}
