/*!
 * $Id: request.cc 149 2008-09-10 05:31:35Z sriramsrao $
 *
 * \file request.cc
...
static bool file_exists(fid_t fid)
{
	return metatree.getFattr(fid) != NULL;
}

static bool path_exists(const string &pathname)
{
	MetaFattr *fa = metatree.lookupPath(KFS::ROOTFID, pathname);
	return fa != NULL;
}

static bool is_dir(fid_t fid)
{
	MetaFattr *fa = metatree.getFattr(fid);
	return fa != NULL && fa->type == KFS_DIR;
}

static void AddCounter(const char *name, MetaOp opName)
{
...
static void UpdateCounter(MetaOp opName)
{
	Counter *c;
 ...
 */
void KFS::ChangeIncarnationNumber(MetaRequest *r)
{
	if (chunkVersionInc < 1)
		// disable this bumping for now
		++chunkVersionInc;
	MetaChangeChunkVersionInc *ccvi = new MetaChangeChunkVersionInc(
			chunkVersionInc, r);
 ...
 */
void KFS::setClusterKey(const char *key)
{
	gClusterKey = key;
}
 ...
 */
static void handle_lookup(MetaRequest *r)
{
...
static void handle_lookup_path(MetaRequest *r)
{
...
static void handle_create(MetaRequest *r)
{
	...
	req->status = metatree.create(req->dir, req->name, &fid, req->numReplicas,
			req->exclusive);
	req->fid = fid;
}

static void handle_mkdir(MetaRequest *r)
{
 ...
 */
static bool isWormMutationAllowed(const string &pathname)
{
...
static void handle_remove(MetaRequest *r)
{
...
static void handle_rmdir(MetaRequest *r)
{
...
static void handle_readdir(MetaRequest *r)
{
...
public:
	EnumerateLocations(vector<ServerLocation> &result) :
		v(result)
	{
	}
...
class ListServerLocations
{
	ostringstream &os;
public:
	ListServerLocations(ostringstream &out) :
		os(out)
	{
	}
...
class EnumerateReaddirPlusInfo
{
	ostringstream &os;
public:
	EnumerateReaddirPlusInfo(ostringstream &o) :
		os(o)
	{
	}
	void operator()(MetaDentry *entry)
	{
		...
		os << "Replicas: ";
		for_each(l.locations.begin(), l.locations.end(),
				ListServerLocations(os));
		os << "\r\n";
	}

};

static void handle_readdirplus(MetaRequest *r)
{
 ...
 */
static void handle_getalloc(MetaRequest *r)
{
 ...
 */
static void handle_getlayout(MetaRequest *r)
{
 ...
 */
static void handle_allocate(MetaRequest *r)
{
	...
	if (!req->layoutDone)
	{
		...
		req->status = metatree.allocateChunkId(req->fid, req->offset,
				&req->chunkId, &req->chunkVersion, &req->numReplicas);
	...
	if (req->status != 0)
	{
		...
		metatree.getChunkVersion(req->fid, req->chunkId, &req->chunkVersion);
	...
	// merry way.
	req->status = metatree.assignChunkId(req->fid, req->offset, req->chunkId,
			req->chunkVersion);
	if (req->status != 0)
		KFS_LOG_VA_DEBUG("Assign chunk id failed for %lld,%lld", req->fid, req->offset);
}

static void handle_truncate(MetaRequest *r)
{
...
static void handle_rename(MetaRequest *r)
{
	MetaRename *req = static_cast<MetaRename *> (r);
	if (gWormMode && ((!isWormMutationAllowed(req->oldname)) || path_exists(
			req->newname)))
	{
...
static void handle_change_file_replication(MetaRequest *r)
{
	MetaChangeFileReplication *req =
			static_cast<MetaChangeFileReplication *> (r);
	if (file_exists(req->fid))
		req->status
				= metatree.changeFileReplication(req->fid, req->numReplicas);
	else
		req->status = -ENOENT;
}

static void handle_retire_chunkserver(MetaRequest *r)
{
...
static void handle_toggle_rebalancing(MetaRequest *r)
{
...
static void handle_execute_rebalanceplan(MetaRequest *r)
{
...
static void handle_log_rollover(MetaRequest *r)
{
	r->status = 0;
}

static void handle_hello(MetaRequest *r)
{
...
static void handle_bye(MetaRequest *r)
{
...
static void handle_lease_acquire(MetaRequest *r)
{
...
static void handle_lease_renew(MetaRequest *r)
{
...
static void handle_lease_cleanup(MetaRequest *r)
{
...
static void handle_chunk_corrupt(MetaRequest *r)
{
...
static void handle_chunk_replication_check(MetaRequest *r)
{
	MetaChunkReplicationCheck *req =
			static_cast<MetaChunkReplicationCheck *> (r);

	gLayoutManager.ChunkReplicationChecker();
	req->status = 0;
}

static void handle_chunk_size_done(MetaRequest *r)
{
	...
	if ((fa != NULL) && (fa->type == KFS_FILE))
	{
		...
		{
			fa->filesize = (fa->chunkcount - 1) * CHUNKSIZE + req->chunkSize;
		}
	}
	req->status = 0;
}

static void handle_chunk_replication_done(MetaRequest *r)
{
...
static void handle_change_chunkVersionInc(MetaRequest *r)
{
	r->status = 0;
}

static void handle_ping(MetaRequest *r)
{
	MetaPing *req = static_cast<MetaPing *> (r);

	req->status = 0;

	gLayoutManager.Ping(req->systemInfo, req->servers, req->downServers,
			req->retiringServers);

}

static void handle_dump_chunkToServerMap(MetaRequest *r)
{
...
static void handle_stats(MetaRequest *r)
{
...
static void handle_open_files(MetaRequest *r)
{
...
static void setup_handlers()
{
	...
	gParseHandlers["CHANGE_FILE_REPLICATION"]
			= parseHandlerChangeFileReplication;
 ...
 */
void KFS::initialize_request_handlers()
{
	setup_handlers();
}
...
///
void KFS::submit_request(MetaRequest *r)
{
 ...
 */
int MetaLookup::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaLookupPath::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaCreate::log(ofstream &file) const
{
	file << "create/dir/" << dir << "/name/" << name << "/id/" << fid
 ...
 */
int MetaMkdir::log(ofstream &file) const
{
	file << "mkdir/dir/" << dir << "/name/" << name << "/id/" << fid << '\n';
	return file.fail() ? -EIO : 0;
}
 ...
 */
int MetaRemove::log(ofstream &file) const
{
 ...
 */
int MetaRmdir::log(ofstream &file) const
{
 ...
 */
int MetaReaddir::log(ofstream &file) const
{
	return 0;
}
 ...
 * \brief log directory read (nop)
 */
int MetaReaddirPlus::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaGetalloc::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaGetlayout::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaAllocate::log(ofstream &file) const
{
	file << "allocate/file/" << fid << "/offset/" << offset << "/chunkId/"
			<< chunkId << "/chunkVersion/" << chunkVersion << '\n';
	return file.fail() ? -EIO : 0;
}
 ...
 */
int MetaTruncate::log(ofstream &file) const
{
 ...
 * \brief log a rename
 */
int MetaRename::log(ofstream &file) const
{
	file << "rename/dir/" << dir << "/old/" << oldname << "/new/" << newname
			<< '\n';
	return file.fail() ? -EIO : 0;
}
 ...
 */
int MetaChangeChunkVersionInc::log(ofstream &file) const
{
 ...
 */
int MetaChangeFileReplication::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaRetireChunkserver::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaToggleRebalancing::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaExecuteRebalancePlan::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaLogRollover::log(ofstream &file) const
{
 ...
 */
int MetaHello::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaBye::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaChunkAllocate::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaChunkDelete::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaChunkTruncate::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaChunkHeartbeat::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaChunkStaleNotify::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaChunkRetire::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaChunkCorrupt::log(ofstream &file) const
{
	return 0;
}
 ...
 * nothing to log.
 */
int MetaChunkVersChange::log(ofstream &file) const
{
	return 0;
}
 ...
 * nothing to log.
 */
int MetaChunkReplicate::log(ofstream &file) const
{
	return 0;
}
 ...
 * nothing to log.
 */
int MetaChunkSize::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaPing::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaStats::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaDumpChunkToServerMap::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaOpenFiles::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaLeaseAcquire::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaLeaseRenew::log(ofstream &file) const
{
	return 0;
}
 ...
 */
int MetaLeaseCleanup::log(ofstream &file) const
{
	return 0;
}
 ...
 * nothing to log.
 */
int MetaChunkReplicationCheck::log(ofstream &file) const
{
	return 0;
}
 ...
 */
/// 解析cmdBuf中的命令，存放到res中
int KFS::ParseCommand(char *cmdBuf, int cmdLen, MetaRequest **res)
{
	const char *delims = " \r\n";
...
static int parseHandlerCreate(Properties &prop, MetaRequest **r)
{
	...
	if (name == NULL)
		return -1;
	// cap replication
	numReplicas = min((int16_t) prop.getValue("Num-replicas", 1),
 ...
 */
int parseHandlerLeaseAcquire(Properties &prop, MetaRequest **r)
{
	seq_t seq = prop.getValue("Cseq", (seq_t) -1);
 ...
 */
int parseHandlerLeaseRenew(Properties &prop, MetaRequest **r)
{
	seq_t seq = prop.getValue("Cseq", (seq_t) -1);
	chunkId_t chunkId = prop.getValue("Chunk-handle", (chunkId_t) -1);
 ...
 */
int parseHandlerChunkCorrupt(Properties &prop, MetaRequest **r)
{
	seq_t seq = prop.getValue("Cseq", (seq_t) -1);
 ...
 */
int parseHandlerPing(Properties &prop, MetaRequest **r)
{
	seq_t seq = prop.getValue("Cseq", (seq_t) -1);
 ...
 */
int parseHandlerStats(Properties &prop, MetaRequest **r)
{
	seq_t seq = prop.getValue("Cseq", (seq_t) -1);
 ...
 */
int parseHandlerDumpChunkToServerMap(Properties &prop, MetaRequest **r)
{
	seq_t seq = prop.getValue("Cseq", (seq_t) -1);
 ...
 * \brief Parse out the headers from a STATS message.
 */
int parseHandlerOpenFiles(Properties &prop, MetaRequest **r)
{
	seq_t seq = prop.getValue("Cseq", (seq_t) -1);
 ...
 */
void MetaLookup::response(ostringstream &os)
{
	static string fname[] =
	{ "empty", "file", "dir" };
...
void MetaLookupPath::response(ostringstream &os)
{
	static string fname[] =
	{ "empty", "file", "dir" };
...
void MetaCreate::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n";
...
void MetaRemove::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n\r\n";
...
void MetaMkdir::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n";
...
void MetaRmdir::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n\r\n";
...
void MetaReaddir::response(ostringstream &os)
{
...
void MetaReaddirPlus::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n";
...
void MetaRename::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n\r\n";
...
void MetaGetalloc::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n";
...
void MetaGetlayout::response(ostringstream &os)
{
...
class PrintChunkServerLocations
{
	ostringstream &os;
public:
	PrintChunkServerLocations(ostringstream &out) :
		os(out)
	{
	}
...
void MetaAllocate::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n";
...
void MetaLeaseAcquire::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n";
...
void MetaLeaseRenew::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n\r\n";
...
void MetaHello::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n\r\n";
...
void MetaChunkCorrupt::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n\r\n";
...
void MetaTruncate::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n\r\n";
...
void MetaChangeFileReplication::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
...
void MetaRetireChunkserver::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n\r\n";
...
void MetaToggleRebalancing::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n\r\n";
...
void MetaExecuteRebalancePlan::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n\r\n";
...
void MetaPing::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n";
...
void MetaStats::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n";
...
void MetaDumpChunkToServerMap::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n\r\n";
...
void MetaOpenFiles::response(ostringstream &os)
{
	os << "OK\r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Status: " << status << "\r\n";
 ...
 * @param[out] os: A string stream that contains the response.
 */
void MetaChunkAllocate::request(ostringstream &os)
{
...
void MetaChunkDelete::request(ostringstream &os)
{
	os << "DELETE \r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Version: KFS/1.0\r\n";
...
void MetaChunkTruncate::request(ostringstream &os)
{
	os << "TRUNCATE \r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Version: KFS/1.0\r\n";
...
void MetaChunkHeartbeat::request(ostringstream &os)
{
	os << "HEARTBEAT \r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Version: KFS/1.0\r\n\r\n";
...
void MetaChunkStaleNotify::request(ostringstream &os)
{
	string s;
...
void MetaChunkRetire::request(ostringstream &os)
{
	os << "RETIRE \r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Version: KFS/1.0\r\n\r\n";
...
void MetaChunkVersChange::request(ostringstream &os)
{
...
void MetaChunkReplicate::request(ostringstream &os)
{
	os << "REPLICATE \r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Version: KFS/1.0\r\n";
...
void MetaChunkSize::request(ostringstream &os)
{
	os << "SIZE \r\n";
	os << "Cseq: " << opSeqno << "\r\n";
	os << "Version: KFS/1.0\r\n";
