//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: KfsModulePy.cc 138 2008-08-26 03:58:00Z sriramsrao $
//
// Created 2006/08/01
...
static PyMemberDef
		Client_members[] =
		{
		{ "properties", T_OBJECT, offsetof(kfs_Client, propfile), RO,
				"properties file" },
				{ "cwd", T_OBJECT, offsetof(kfs_Client, cwd), RO,
						"current directory" },
				{ NULL } };
...
static void File_dealloc(PyObject *pself)
{
...
static int modeflag(const char *modestr)
{
...
static int set_file_members(kfs_File *self, const char *path,
...
static int File_init(PyObject *pself, PyObject *args, PyObject *kwds)
{
	kfs_File *self = (kfs_File *) pself;
	char *nm, *md = "r";
	PyObject *cl;
	static char *kwlist[] =
	{ "client", "name", "mode", NULL };

	int ok = !PyArg_ParseTupleAndKeywords(args, kwds, "O!s|s", kwlist,
			&kfs_ClientType, &cl, &nm, &md);
	if (!ok)
		return -1;
...
{
	kfs_File *self = (kfs_File *) pself;
	return PyString_FromFormat("kfs.file<%s, %s, %d>", PyString_AsString(
			self->name), PyString_AsString(self->mode), self->fd);
}

static int File_print(PyObject *pself, FILE *fp, int flags)
{
	kfs_File *self = (kfs_File *) pself;
	fprintf(fp, "kfs.file<%s, %s, %d>\n", PyString_AsString(self->name),
			PyString_AsString(self->mode), self->fd);
	return 0;
}
...
kfs_dataVerify(PyObject *pself, PyObject *args)
{
	...
	bool res = cl->client->VerifyDataChecksums(self->fd, 0, buf,
			(ssize_t) wsize);
...
{ "chunk_locations", kfs_chunkLocations, METH_VARARGS,
		"Get location(s) of a chunk." },
...
{ "data_verify", kfs_dataVerify, METH_VARARGS,
		"Verify data matches what is in KFS." },
{ NULL } };
...
{
	kfs_Client *self = (kfs_Client *) pself;
	return PyString_FromFormat("kfs.client<%s, %s>", PyString_AsString(
			self->propfile), PyString_AsString(self->cwd));
}
...
static string strip_dots(string path)
{
 ...
 */
static string build_path(PyObject *cwd, const char *input)
{
	string tail(input);
	if (input[0] == '/')
...
kfs_readdir(PyObject *pself, PyObject *args)
{
	...
	for (int i = 0; i != n; i++)
	{
		PyTuple_SetItem(tuple, i, PyString_FromString(result[i].c_str()));
	}
	return tuple;
}
...
package_fattr(KfsFileAttr &fa)
{
	...
	PyTuple_SetItem(tuple, 5, PyString_FromString(fa.isDirectory ? "dir"
			: "file"));
...
kfs_open(PyObject *pself, PyObject *args)
{
	...
	kfs_File *f = (kfs_File *) kfs_FileType.tp_new(&kfs_FileType, NULL, NULL);
	if (f == NULL || set_file_members(f, path.c_str(), modestr, self, -1) < 0)
	{
		return NULL;
	}
	return (PyObject *) f;
...
PyMODINIT_FUNC initkfs()
{
	if (PyType_Ready(&kfs_ClientType) < 0 || PyType_Ready(&kfs_FileType) < 0)
		return;
